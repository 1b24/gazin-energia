...
/**
 * Modal de importação Excel para Filiais.
 *
 * Fluxo:
 *  1. Usuário clica "Importar Excel" no header da tabela → abre este dialog.
 *  2. Escolhe arquivo .xlsx (drag-drop ou input). Lê como ArrayBuffer.
 *  3. Envia pra `importFilial(buffer)` (server action).
 *  4. Renderiza resultado:
 *     - `ok=false`: lista agrupada por linha, com campo, valor, esperado.
 *     - `ok=true`: contadores criados/atualizados + botão Fechar (revalidate
 *       já roda no server, a tabela atualiza ao fechar via router.refresh).
 *
 * All-or-nothing: o botão "Aplicar" desaparece e só temos "Tentar de novo"
 * quando há erros. O usuário corrige na planilha e reimporta.
 */
...
import { importFilial, type ImportFilialResult } from "./actions";
...
export function FilialImportDialog({ open, onOpenChange }: Props) {
  const [file, setFile] = useState<File | null>(null);
  const [result, setResult] = useState<ImportFilialResult | null>(null);
  ...
  function handleClose(open: boolean) {
    if (!open) {
      reset();
      // Se importou com sucesso, refresca a página pra mostrar dados novos.
  ...
  async function handleApply() {
    ...
    startTransition(async () => {
      const r = await importFilial(buffer);
      setResult(r);
    });
  }

  // Agrupa erros por linha pra UX legível.
  type Errs = ImportFilialResult["errors"];
