...
/**
 * Modal de importação Excel para Injeções.
 *
 * Espelha `app/(dashboard)/consumo/import-dialog.tsx`. Diferenças:
 *  - Avisos específicos sobre Fornecedor (lookup por nome, ambiguidade).
 *  - Texto adaptado pra entidade.
 *
 * Terceiro dialog idêntico — depois do próximo (geração ou venda) vale
 * abstrair em `<EntityImportDialog action={...} entityLabel={...} />`.
 */
...
import { importInjecao, type ImportInjecaoResult } from "./actions";
...
export function InjecaoImportDialog({ open, onOpenChange }: Props) {
  const [file, setFile] = useState<File | null>(null);
  const [result, setResult] = useState<ImportInjecaoResult | null>(null);
  ...
  async function handleApply() {
    ...
    startTransition(async () => {
      const r = await importInjecao(buffer);
      setResult(r);
    });
  }

  type Errs = ImportInjecaoResult["errors"];
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle>Importar planilha Excel — Injeção</DialogTitle>
        </DialogHeader>
        ...
        {!result && (
          <div className="flex flex-col gap-4">
            <div className="space-y-2 text-sm text-muted-foreground">
              ...
              <p>
                <strong>Fornecedor</strong> (opcional): preencha{" "}
                <strong>&quot;Fornecedor Nome&quot;</strong>. Se houver dois
                fornecedores com mesmo nome, o sistema avisa e pede o ID.
              </p>
              <p>
                <strong>Duplicata</strong>: combinação <em>Filial + Ano + Mês</em>{" "}
                não pode repetir (use o ID pra atualizar existente).
              </p>
              <p className="text-xs">
                Limite: 5000 linhas por importação.
              </p>
            </div>
            <input
              type="file"
