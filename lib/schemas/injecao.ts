 ...
 *
 * Sem FKs formais: `filialDescricao` e `fornecedorRaw` são strings livres
 * (a fonte legada usa rótulos descritivos como "Auto Posto 04900",
 * "COTESA", "SERENA" — não códigos numéricos nem cuids).
 */
import { z } from "zod";
...
export const injecaoSchema = z.object({
  ...
  filialDescricao: z.preprocess(nullishToNull, z.string().nullable()),
  fornecedorRaw: z.preprocess(nullishToNull, z.string().nullable()),
...
export const injecaoFormFields: FormFieldConfig[] = [
  {
    name: "filialDescricao",
    label: "Filial (descrição)",
    type: "text",
    span: 2,
    placeholder: "Ex: Auto Posto 04900",
  },
  ...
  { name: "fornecedorRaw", label: "Fornecedor", type: "text", span: 2, placeholder: "COTESA, SERENA, ..." },
